...
configFileName = "configurations/PEMS08_FL_astgcn.conf"
config = configparser.ConfigParser()
print('Read configuration file: %s' % (configFileName))
...
print('adj_mx:', adj_mx, 'distance_mx:', distance_mx)
...
def train_main():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if masked_flag:
            val_loss = compute_val_loss_mstgcn(net, val_loader, criterion_masked, masked_flag,missing_value,sw, epoch,_mean, _std)
        else:
            val_loss = compute_val_loss_mstgcn(net, val_loader, criterion, masked_flag, missing_value, sw, epoch,_mean, _std)
        ...
        for batch_index, batch_data in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            encoder_inputs = encoder_inputs * _std[0, 0, 0, 0] + _mean[0, 0, 0, 0]
...
def predict_main(global_step, data_loader, data_target_tensor,metric_method, _mean, _std, type):
    ...
    last_predict_data = []
    with torch.no_grad():
        test_data = torch.rand(1, 912, 1, 12).to(DEVICE)
        for batch_index, batch_data in enumerate(test_loader):
            encoder_inputs, label = batch_data
            encoder_inputs = encoder_inputs * _std[0, 0, 0, 0] + _mean[0, 0, 0, 0]
            output = net(encoder_inputs)
            pass
            last_predict_data = output
...
if __name__ == "__main__":
    #pass
    train_main()
    predict_main(216, test_loader, test_target_tensor,metric_method, _mean, _std, 'test')
